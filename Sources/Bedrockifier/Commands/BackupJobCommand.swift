...
public final class BackupJobCommand: Command {
    ...
    public func run(using context: CommandContext, signature: Signature) throws {
        ...
        for (serverContainer, serverWorldsPath) in config.servers {
            ...
            try WorldBackup.makeBackup(backupUrl: backupUrl, dockerPath: dockerPath, containerName: serverContainer, worldsPath: worldsUrl)
        }
        
        print("Performing Trim Jobs")
        if let trimJob = config.trim {
            try WorldBackup.trimBackups(at: backupUrl, dryRun: false, trimDays: trimJob.trimDays, keepDays: trimJob.keepDays, minKeep: trimJob.minKeep)
        }
    }
}
